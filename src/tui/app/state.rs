...
pub struct App {
    ...
    pub detailed_task_cache: HashMap<String, Task>,
    ...
    pub filtered_projects: Vec<(String, String)>,
    ...
    pub filtered_filters: Vec<(String, String)>,
    pub selected_filter_picker_index: usize,
    pub filters: Vec<(String, String)>,
    pub filter_descriptions: std::collections::HashMap<String, String>,
    ...
    pub filtered_subtask_tasks: Vec<(String, String)>,
    pub selected_subtask_picker_index: usize,
    pub selected_subtask_task_ids: Vec<String>,
...
impl App {
    ...
    pub fn open_project_picker_from_form(&mut self) {
        ...
        if let Some(ref form) = self.form_edit_state {
            self.current_project_id = Some(form.project_id.clone());
        }
    ...
    pub fn default() -> Self {
        Self::new_with_config(OpusConfig::default(), "Inbox".to_string())
    }
    ...
    pub fn get_detailed_task(&self, task_id: &str) -> Option<&Task> {
        self.detailed_task_cache.get(task_id)
    }

    pub fn cache_detailed_task(&mut self, task: Task) {
        self.detailed_task_cache.insert(task.id.clone(), task);
    }
    ...
    pub fn add_debug_message(&mut self, message: String) {
        ...
        // Only log to file if OPUS_DEBUG is set
        if std::env::var("OPUS_DEBUG").is_ok() {
            if let Ok(mut file) = OpenOptions::new().create(true).append(true).open("opus_debug.log") {
    ...
    fn task_to_magic_syntax(&self, task: &crate::opus::models::Task) -> String {
        ...
        if let Some(assignees) = &task.assignees {
            for assignee in assignees {
                result.push_str(&format!(" @{}", assignee.name));
            }
        }
    ...
    pub fn update_suggestions(&mut self, input: &str, cursor: usize) {
        ...
        if let Some(pos) = before_cursor.rfind('+') {
            ...
            if suggestion_text.chars().all(is_suggestion_char) {
                ...
                let projects: Vec<_> = self.project_map.values()
                    .cloned()
                    .collect();
    ...
    pub fn apply_sort(&mut self, sort: SortOrder) {
        ...
        match sort {
            ...
            SortOrder::PriorityHighToLow => self.tasks.sort_by(|a, b| priority_to_num(&b.priority).cmp(&priority_to_num(&a.priority))),
            SortOrder::PriorityLowToHigh => self.tasks.sort_by(|a, b| priority_to_num(&a.priority).cmp(&priority_to_num(&b.priority))),
        ...
        self.tasks.sort_by(|a, b| {
            for (column, sort_config) in sort_columns {
                let ordering = match column.column_type {
                    ...
                    TaskColumn::Priority => {
                        let a_num = priority_to_num(&a.priority);
                        let b_num = priority_to_num(&b.priority);
                        let cmp = match (a_num, b_num) {
                            (0, 0) => std::cmp::Ordering::Equal,
                            (0, _) => std::cmp::Ordering::Greater,
                            (_, 0) => std::cmp::Ordering::Less,
                            _ => match sort_config.direction {
                                SortDirection::Asc => a_num.cmp(&b_num),
                                SortDirection::Desc => b_num.cmp(&a_num),
                            },
                        };
                        cmp
                    }
        ...
        let result = match action.action.as_str() {
            "project" => {
                let project_id = self.project_map.iter().find_map(|(id, name)| {
                    if name == &action.target { Some(id.clone()) } else { None }
                });
            ...
            "priority" => {
                match action.target.parse::<Priority>() {
                    Ok(priority) => {
                        task.priority = priority;
                        Ok(())
                    }
                    Err(_) => Err(format!("Invalid priority '{}': use low, medium, high, urgent", action.target))
                }
            },
            "label" => {
                ...
                if let Some(lid) = label_id {
                    ...
                    let new_label = crate::opus::models::Label {
                        id: lid.clone(),
                        name: action.target.clone(),
                        color: self.label_colors.get(&lid).cloned().unwrap_or_default(),
                        task_id: None,
                        workspace_id: None,
                        created_at: now,
                        updated_at: now,
                    ...
                    if let Some(ref mut labels) = task.labels {
                        ...
                        task.labels = Some(vec![new_label]);
                    }
                    Ok(())
                } else {
        ...
        if result.is_ok() {
            if let Some(all_task) = self.all_tasks.iter_mut().find(|t| t.id == task_id) {
                ...
                match action.action.as_str() {
                    "project" => all_task.project_id = task.project_id.clone(),
                    "priority" => all_task.priority = task.priority.clone(),
    ...
    pub fn cycle_filter_backward(&mut self) {
        ...
        self.current_filter_id = Some(self.filters[new_idx].0.clone());
    ...
    pub fn set_filters_with_descriptions(&mut self, filters: Vec<(String, String)>, descriptions: std::collections::HashMap<String, String>) {
        ...
        if self.subtask_picker_input.is_empty() {
            ...
            let mut scored_tasks: Vec<_> = self.all_tasks
                ...
                .filter_map(|task| {
                    ...
                    if score > 0.0 {
                        Some((task.id.clone(), task.title.clone(), score))
                    } else {
                        None
                    }
                })
    ...
    pub fn get_selected_subtask_task(&self) -> Option<(String, String)> {
    ...
    pub fn toggle_subtask_task_selection(&mut self) {
        if let Some((task_id, _)) = self.get_selected_subtask_task() {
            if self.selected_subtask_task_ids.iter().any(|id| id == &task_id) {
    ...
    pub fn is_subtask_task_selected(&self, task_id: &str) -> bool {
        self.selected_subtask_task_ids.iter().any(|id| id == task_id)
    }

    /// Get visual indicator for subtask relationships
    pub fn get_task_relation_indicator(&self, task: &crate::opus::models::Task) -> Option<&'static str> {
    ...
    pub fn apply_hierarchical_sort(&mut self) {
        ...
        for task in &self.tasks {
            if let Some(ref related_tasks) = task.related_tasks {
                if let Some(subtasks) = related_tasks.get("subtask") {
                    ...
                    parent_child_map.insert(task.id.clone(), subtask_ids.clone());

                    for subtask_id in subtask_ids {
                        child_parent_map.insert(subtask_id, task.id.clone());
                    }
                }
            }
        }
        ...
        for task in &self.tasks {
            ...
            if parent_child_map.contains_key(&task.id) {
                ...
                processed_tasks.insert(task.id.clone());
                ...
                if let Some(subtask_ids) = parent_child_map.get(&task.id) {
                    for subtask_id in subtask_ids {
                        if let Some(subtask) = self.tasks.iter().find(|t| t.id == *subtask_id) {
                            ...
                            processed_tasks.insert(subtask_id.clone());
                        }
                    }
                }
    ...
    pub fn get_task_hierarchy_info(&self, task: &crate::opus::models::Task) -> (usize, &'static str) {
        if let Some(ref related_tasks) = task.related_tasks {
    ...
    pub fn is_subtask_of(&self, task: &crate::opus::models::Task, parent_id: &str) -> bool {
        if let Some(ref related_tasks) = task.related_tasks {
    ...
    pub fn get_subtasks_of(&self, parent_id: &str) -> Vec<&crate::opus::models::Task> {
        self.tasks.iter()
    ...
    pub fn get_parent_of(&self, task: &crate::opus::models::Task) -> Option<&crate::opus::models::Task> {
        if let Some(ref related_tasks) = task.related_tasks {
            if let Some(parent_tasks) = related_tasks.get("subtask") {
    ...
    // pub fn get_task_relation_indicator(&self, _task: &crate::opus::models::Task) -> Option<RelationKind> {
