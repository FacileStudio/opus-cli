...
fn create_wrapped_cell_for_column<'a>(
    ...
    match &column.column_type {
        TaskColumn::Title => {
            ...
            }
            {
                let color = match &task.priority {
                    ...
                    Priority::High => Some(Color::Rgb(255, 165, 0)),
                    Priority::Medium => Some(Color::Yellow),
                    Priority::Low => Some(Color::Blue),
                    Priority::NoPriority => None,
                };
                if let Some(c) = color {
                    spans.push(Span::styled("\u{f024} ", Style::default().fg(c)));
                }
            }
        ...
        TaskColumn::Priority => {
            ...
            match &task.priority {
                Priority::Urgent => Cell::from(format!("{}urgent", flag_icon)).style(Style::default().fg(Color::Red)),
                Priority::High => Cell::from(format!("{}high", flag_icon)).style(Style::default().fg(Color::Rgb(255, 165, 0))),
                Priority::Medium => Cell::from(format!("{}medium", flag_icon)).style(Style::default().fg(Color::Yellow)),
                Priority::Low => Cell::from(format!("{}low", flag_icon)).style(Style::default().fg(Color::Blue)),
...
pub fn draw_tasks_table(f: &mut Frame, app: &App, area: Rect) {
    ...
    let rows = app.tasks.iter().enumerate()
        ...
        .map(|(i, task)| {
            ...
            if let (Some(flash_id), Some(start_time)) = (app.flash_task_id.clone(), app.flash_start) {
                if task.id == flash_id {
